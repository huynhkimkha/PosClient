...
@Component({
    selector: 'app-update-importing-material',
    templateUrl: './app-update-importing-material.component.html'
})
export class AppUpdateImportingMaterialComponent implements AfterViewInit {
    ...
    public importingMaterialSelected: ImportingMaterialModel = new ImportingMaterialModel();
    ...
    public show(importingMaterial: ImportingMaterialModel) {
        this.importingMaterialSelected = new ImportingMaterialModel(importingMaterial);
        this.loadImportingMaterialFull();
    ...
    public saveImportingMaterial() {
        ...
        this.importingMaterialService.update(this.importingMaterial).subscribe(response => this.saveImportingMaterialCompleted(response));
    }
    ...
    private saveImportingMaterialCompleted(res: ResponseModel<ImportingMaterialFullModel>) {
        ...
        this.alert.success('Cập nhật phiếu nhập thành công!');
        this.hide();
    ...
    private loadImportingMaterialFull() {
        this.loading.show(this.root.nativeElement.querySelector('.modal-content'));
        this.importingMaterialService.getFullById(this.importingMaterialSelected.id).subscribe(res =>
            this.loadImportingMaterialFullCompleted(res));
    }

    private loadImportingMaterialFullCompleted(res: ResponseModel<ImportingMaterialFullModel>) {
        this.loading.hide(this.root.nativeElement.querySelector('.modal-content'));
        if (res.status !== HTTP_CODE_CONSTANT.OK) {
            res.message.forEach(value => {
                this.alert.error(value);
